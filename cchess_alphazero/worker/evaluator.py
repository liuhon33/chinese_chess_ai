...
class ContinuousEvaluator:
    ...
    def start(self):
        ...
        threshold = float(getattr(self.config.eval, "next_generation_replace_rate", 0.55))
        while True:
            if not is_next_generation_model_fresh(self.config):
                if not waiting_for_candidate:
                    logger.info(
                        "Waiting for candidate model at %s; polling again in %.1f seconds.",
            ...
            logger.info(
                "Starting evaluation: candidate=%s, best=%s",
                self.config.resource.next_generation_weight_path,
...
class EvaluateWorker:
    ...
    def start(self):
        ...
        for idx in range(self.config.eval.game_num):
            ...
            logger.info(
                "进程%s评测完毕 用时%.1f秒, %s回合, %s, 得分：%s, value = %s, idx = %s",
                self.pid,
                end_time - start_time,
                turns / 2,
                result,
    ...
    def start_game(self, idx):
        ...
        if idx % 2 == 0:
            red = self.player1
            black = self.player2
            logger.debug("best model is red, ng is black")
        else:
            red = self.player2
            black = self.player1
            logger.debug("best model is black, ng is red")
